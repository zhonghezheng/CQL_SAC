...
from parameters import alpha, tau, gamma, alpha_prime
import numpy as np

class SAC():
    def __init__(self, state_dim=2, action_dim=1, critic_hidden=255, actor_hidden=255):
    ...
    def update(self, batchsize):
        ...
        q_loss = q1_loss + q2_loss + alpha_prime * min_qf1_loss + alpha_prime *min_qf2_loss
        ...
        phi_loss = (torch.min(q1_predict, q2_predict) - alpha*phi_log).mean()
    ...
    def get_action(self, state):
        phi_actions, phi_log = self.actor(state)
        return phi_actions
